...
export const listPaddockManagerIds = () => {
  const paddockManageIdInOrder = paddockManagers.map((paddock) => paddock.id);
  paddockManageIdInOrder.sort((a, b) => {
    return a - b;
  });
  return paddockManageIdInOrder;
};
...
export const sortPaddockTypeByTotalArea = () => {
  const padType = paddockType.map(({ id, name }) => {
    const pad = paddocks.filter(({ paddockTypeId }) => paddockTypeId === id);
    return { pad, name };
  });
  const areasSum = padType.map(({ pad, name }) => {
    const sumAreas = pad.map(({ area }) => area).reduce((a, b) => a + b, 0);
    return { sumAreas, name };
  });
  areasSum.sort((a, b) => {
    const nameA = b.sumAreas;
    const nameB = a.sumAreas;
    if (nameA < nameB) {
      return -1;
    }
    if (nameA > nameB) {
...
export const sortFarmManagerByAdminArea = () => {
  const Managers = paddockManagers.map(({ id, name }) => {
    const padManage = paddocks.filter(
      ({ paddockManagerId }) => paddockManagerId === id
    );
    return { padManage, name };
  });

  const areasByManager = Managers.map(({ padManage, name }) => {
    const sumAreas = padManage
      .map(({ area }) => area)
      .reduce((a, b) => a + b, 0);
    return { sumAreas, name };
  });
  areasByManager.sort((a, b) => {
    const nameA = b.sumAreas;
    const nameB = a.sumAreas;
    if (nameA < nameB) {
      return -1;
  ...
  return areasByManager.map(({ name }) => name);
};
...
export const farmManagerNames = () => {
  let farmsBymanagers = {};
  farms.forEach(({ name, id }) => {
    const pads = paddocks.filter(({ farmId }) => farmId === id);
    const padsManagers = pads.map(({ paddockManagerId }) =>
      paddockManagers.find(({ id }) => id === paddockManagerId)
    );
    farmsBymanagers[name] = padsManagers
      .sort((a, b) => a.name.localeCompare(b.name))
      .map(({taxNumber}) =>taxNumber);
      farmsBymanagers[name] = [...new Set(farmsBymanagers[name])];
  });
  return farmsBymanagers
};
