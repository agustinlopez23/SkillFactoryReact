...
export const startDeleteImg = () => {
  return async (dispatch, getState) => {
    const { uid } = getState().auth;
    const { activeNote } = getState().note;
    const { imagesUrls } = activeNote;
    imagesUrls.map(async ({ path }) => {
      // Create a reference to the file to delete
      const desertRef = await ref(FirebaseStorage, path);
      const noteToFireStore = { ...activeNote, imagesUrls: [] };
      delete noteToFireStore.id;

      const docRef = doc(FirebaseDB, `${uid}/notes/note/${activeNote.id}`);

      await setDoc(docRef, noteToFireStore, { merge: true });
      dispatch(updateNoteWithoutSave(activeNote));

      dispatch(deleteNoteByImage());
      dispatch(startSaveNote());
      console.log("logref", desertRef);
      // Delete the file
